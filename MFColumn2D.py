...
class MFColumn_2D:
    ...
    def find_bay_and_storey_for_beams(beam_connectivity):
        ...
        bay = MFColumn_2D.nth_digit(first_node, 1)
        storey = MFColumn_2D.nth_digit(first_node, 2,2)
    ...
    def find_axis_and_storey_for_columns(column_connectivity):
        ...
        axis = MFColumn_2D.nth_digit(first_node, 1)
        storey = MFColumn_2D.nth_digit(first_node, 2,2) + 1  
...
###################### The code below is written so as to change the order of elements for direct comparison with Ziemian results
        self.sorted_column_connectivity= sorted(
                        self.column_connectivity,key=lambda col: tuple(reversed(MFColumn_2D.find_axis_and_storey_for_columns(col[1:])))
                        )
        ...
        self.sorted_beam_connectivity=sorted(self.beam_connectivity,key=lambda b: MFColumn_2D.find_bay_and_storey_for_beams(b[1:])[1])
    ...
    def bay_i_internal_floor_nodes(self, i):
        ...
        return [
            ...
            if len(str(node[0])) == 5
            and MFColumn_2D.nth_digit(node[0], 1) == i
            and MFColumn_2D.nth_digit(node[0], 2,2) != self.no_of_stories
        ]
    ...
    def build_ops_model(self):
        ...
        for i, column_ij_node in enumerate(self.column_connectivity):
            ...
            axis, storey = MFColumn_2D.find_axis_and_storey_for_columns(column_ij_node[1:])
        ...
        for i, beam_ij_node in enumerate(self.beam_connectivity):
            bay, storey = MFColumn_2D.find_bay_and_storey_for_beams(beam_ij_node[1:])
        ...
        for beams in self.beam_connectivity:
            if MFColumn_2D.nth_digit(beams[1],2,2)==self.no_of_stories:
...
###########################################################################################################################
    ...
    def run_load_controlled_anlaysis(self,**kwargs):
        ...
        def find_limit_point():
            if MFColumn_2D.print_ops_status:
    ...
    #     ops.analyze(steps)    
    #     MFColumn_2D.generate_clean_csv_file_from_messy_out_files(filename)
